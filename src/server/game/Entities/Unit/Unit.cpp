...
{
    if (!obj || !IsInMap(obj) || !CanSeeInPhaseContexts(obj))
        return false;
...
void Unit::RemoveNotOwnSingleTargetAuras(bool onPhaseChange /*= false*/)
{
    ...
    for (AuraMap::iterator iter = m_ownedAuras.begin(); iter != m_ownedAuras.end();)
    {
        ...
        if (aura->GetCasterGUID() != GetGUID() && aura->IsSingleTarget())
        {
            ...
            {
                Unit* caster = aura->GetCaster();
                if (!caster || !caster->CanSeeInPhaseContexts(this))
                    RemoveOwnedAura(iter);
                else
                    ++iter;
            }
    ...
    {
        Aura* aura = *iter;
        if (aura->GetUnitOwner() != this && (!onPhaseChange || !aura->GetUnitOwner()->CanSeeInPhaseContexts(this)))
        {
            aura->Remove();
            iter = scAuras.begin();
        }
