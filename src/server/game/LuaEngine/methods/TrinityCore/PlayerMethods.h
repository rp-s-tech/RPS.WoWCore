...
namespace LuaPlayer
{
    ...
    {
        E->Push(player->IsNeverVisibleFor(player, true));
        return 1;
    }
    ...
    {
        E->Push(player->GetPrimarySpecializationEntry()); // ?? Need check
        return 1;
    }
    ...
    {
        E->Push(player->GetPower(POWER_COMBO_POINTS));
        return 1;
    }
    ...
    {
        E->Push(player->m_activePlayerData->LifetimeHonorableKills);
        return 1;
    }
    ...
    int SetPlayerLock(Eluna* E, Player* player)
    {
        ...
        if (apply)
        {
            player->SetUnitFlag(UnitFlags(UnitFlags::UNIT_FLAG_PACIFIED | UnitFlags::UNIT_FLAG_FORCE_NAMEPLATE));
            player->SetClientControl(player, 0);
        }
        else
        {
            player->RemoveUnitFlag(UnitFlags(UnitFlags::UNIT_FLAG_PACIFIED | UnitFlags::UNIT_FLAG_FORCE_NAMEPLATE));
    ...
    /**
     * Sets the [Player]s free talent reset cost to the amount specified for the current spec
     *
     * @param uint32 talent reset cost
     */
    int SetFreeTalentPoints(Eluna* E, Player* player)
    {
        uint32 cost = E->CHECKVAL<uint32>(2);

        player->SetTalentResetCost(cost);
    ...
    int SetRestBonus(Eluna* E, Player* player)
    {
        ...
        RestMgr* restMgr = &player->GetRestMgr();
        restMgr->SetRestBonus(RestTypes::REST_TYPE_XP, bonus);
        return 0;
    }
    ...
    {
        uint32 _optionNpc = E->CHECKVAL<uint32>(2); //2
        const char* msg = E->CHECKVAL<const char*>(3); //4
        uint32 _sender = E->CHECKVAL<uint32>(4); //15
        uint32 _intid = E->CHECKVAL<uint32>(5);//16
        bool _code = E->CHECKVAL<bool>(6, false);//10
        const char* _promptMsg = E->CHECKVAL<const char*>(7, "");//12
        uint32 _money = E->CHECKVAL<uint32>(8, 0);//11
        uint8 _lang = E->CHECKVAL<uint8>(9, 0);//16
    ...
    int GossipAddQuests(Eluna* E, Player* player)
    {
        ...
        if (source->GetTypeId() == TYPEID_UNIT)
        {
            ...
            if (unitSource->HasNpcFlag(UNIT_NPC_FLAG_QUESTGIVER))
     ...
     *
     * @param uint32 talentid : ID of the talentid
    ...
    {
        TalentEntry const* spellId = E->CHECKOBJ<TalentEntry>(2);
        uint8 spec = E->CHECKVAL<uint8>(3);
        bool learning = E->CHECKVAL<bool>(4, true);

        if (spec >= MAX_SPECIALIZATIONS)
            E->Push(false);
        else
    ...
    {
        ObjectGuid victimguid = E->CHECKVAL<ObjectGuid>(2);
