/*
 * NobleNext — script loader (registers all NobleNext C++ modules).
 */

void AddSC_NobleNextWeatherCommands();
void AddSC_NobleNextWeatherScripts();
void AddSC_NobleNextDaytimeCommands();
void AddSC_NobleNextGobTeleCommands();
void AddSC_NobleNextGobTeleScripts();
void AddSC_NobleNextGobMoverCommands();
void AddSC_NobleNextCoreCommands();
void AddSC_NobleNextWaypointsCommands();
void AddSC_NobleNextPetCommands();
void AddSC_NobleNextArmyCommands();
void AddSC_NobleNextMasterCommands();
void AddSC_NobleNextMasterScripts();
void AddSC_NobleNextBattleCommands();
void AddSC_NobleNextBattleScripts();
void AddSC_NobleNextRoleStatCommands();

void AddSC_NobleNextScripts()
{
    AddSC_NobleNextWeatherCommands();
    AddSC_NobleNextWeatherScripts();
    AddSC_NobleNextDaytimeCommands();
    AddSC_NobleNextGobTeleCommands();
    AddSC_NobleNextGobTeleScripts();
    AddSC_NobleNextGobMoverCommands();
    AddSC_NobleNextCoreCommands();
    AddSC_NobleNextWaypointsCommands();
    AddSC_NobleNextPetCommands();
    AddSC_NobleNextArmyCommands();
    AddSC_NobleNextMasterCommands();
    AddSC_NobleNextMasterScripts();
    AddSC_NobleNextBattleCommands();
    AddSC_NobleNextBattleScripts();
    AddSC_NobleNextRoleStatCommands();
}
